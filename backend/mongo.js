...
async function retrievePapers(academicYear, semester, branch, subject) {
    const uri = 'mongodb://192.168.56.1:27017/'; // MongoDB connection URI
                    ...
                    for (const subj of semester.subjects) {
                        if (subj.name === subject) {
                            for (const paper of subj.papers) {
                                ...
                                console.log(`PDF file retrieved: ${fileName}`); // Fixed the string interpolation
                            }
                        }
                    }
                }
...
retrievePapers('2nd year', 2, 'AIML', 'Probability'); // Added missing string quotes around the console.log message
